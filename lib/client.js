window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-css:/Users/yanghongxuan/study/dsh-worker/ui-status-label/src/client/StatusLabelRow.module.css.mjs
		const css = ".VgEfdW_row{border-bottom:1px solid var(--dsw-alias-border-l2);align-items:center;gap:8px;padding:16px 0;display:flex}.VgEfdW_rowText{flex-direction:column;flex:1;gap:4px;min-width:0;padding-right:48px;display:flex}.VgEfdW_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.VgEfdW_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;font-weight:400;line-height:18px}.VgEfdW_input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-module-platform);width:200px;height:36px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:18px;outline:none;padding:0 14px;font-size:14px;line-height:22px}.VgEfdW_input:focus{border-color:var(--dsw-alias-label-primary)}.VgEfdW_input::placeholder{color:var(--dsw-alias-label-tertiary)}";
		...
		*/
		function resolveSnapshotStoreModule() {
			...
			throw new Error("dsh-ui-status-label: no snapshot-store module available — expected @deepseek-ai/dsh-client-store (dsh-client >= 0.1.2-alpha) or @deepseek-ai/dsh-client-runtime/client (legacy hosts); in tests, call setSnapshotStoreModuleForTests first");
		}
		/**
		...
		*/
		function createCompatSnapshotStore(initial) {
		...
		*/
		var StatusLabelPolicy = class {
			...
			statusLabel = createCompatSnapshotStore(DEFAULT_STATUS_LABEL);
			host;
			/**
		...
		*/
		const OFFICIAL_FALLBACK_TEXTS = ["Deep diving...", "深度求索中..."];
		...
		*/
		function isOfficialFallbackText(value) {
