window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region \0dsh-css:/tmp/ui-status-label/src/client/StatusLabelRow.module.css.mjs
		const css = ".QGFWoq_row{border-bottom:1px solid var(--dsw-alias-border-l2);align-items:center;gap:8px;padding:16px 0;display:flex}.QGFWoq_rowText{flex-direction:column;flex:1;gap:4px;min-width:0;padding-right:48px;display:flex}.QGFWoq_title{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:400;line-height:22px}.QGFWoq_desc{color:var(--dsw-alias-label-tertiary);font-size:12px;font-weight:400;line-height:18px}.QGFWoq_input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-module-platform);width:200px;height:36px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:18px;outline:none;padding:0 14px;font-size:14px;line-height:22px}.QGFWoq_input:focus{border-color:var(--dsw-alias-label-primary)}.QGFWoq_input::placeholder{color:var(--dsw-alias-label-tertiary)}";
		...
		*/
		var StatusLabelPolicy = class {
			...
			statusLabel = (0, _deepseek_ai_dsh_client_store.createSnapshotStore)(DEFAULT_STATUS_LABEL);
			host;
			/**
		...
		*/
		function apply(ctx) {
			const policy = new StatusLabelPolicy(ctx.configForms.get(STATUS_NAMESPACE));
			ctx.effect(() => {
